...
chrome.spawn = function(options) {
	return new Promise((resolve, reject) => {
		...
		this.chromeChild = spawn(location, this.options.chromeFlags || ['--headless', '--disable-gpu', '--remote-debugging-port=9222', '--remote-debugging-address=0.0.0.0' , '--hide-scrollbars']);
...
chrome.getChromeLocation = function() {
	if (this.options.chromeLocation || process.env.CHROME_LOCATION) {
		return this.options.chromeLocation || process.env.CHROME_LOCATION;
	}
	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Page.loadEventFired(({timestamp}) => {
				tab.prerender.loadEventFired = true;
...
chrome.checkIfPageIsDoneLoading = function(tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			let doneLoading = tab.prerender.lastRequestReceivedAt < ((new Date()).getTime() - waitAfterLastRequest)

			if(this.options.waitUntil == 'domcontentloaded'){
				doneLoading = doneLoading && tab.prerender.domContentEventFired
			} else if(this.options.waitUntil == 'load'){
				doneLoading = doneLoading && tab.prerender.loadEventFired
			} else{
				doneLoading = doneLoading && tab.prerender.numRequestsInFlight <= 0
			}
